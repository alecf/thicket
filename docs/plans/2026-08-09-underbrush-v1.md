...
> **Amended during implementation.** The schema originally written here stored
> L0 and L1 as separate rows keyed by their own normalized hash. That layout
> cannot reconstruct what the clusterer needs, and the amendment is recorded
> here rather than quietly fixed in code, because the reasoning is the whole
> content of this task.

**Files:**
- Create: `src/cache/db.ts`, `src/fingerprint/shape.ts`, `tests/cache.test.ts`, `tests/cache-pipeline.test.ts`
- Modify: `src/fingerprint/cluster.ts`, `src/run.ts`, `src/cli.ts`

### Why one row per fragment, not one row per shape

`findDuplication` files every physical fragment under **both** its L0 and its
L1 hash, and then suppresses an L1 cluster whose members all share a single L0
hash that is already an L0 cluster — that finding is the same one twice.

Storing the two levels as unrelated rows loses the association between them.
Given a cached L1 group there is no way back to the L0 shape each member had,
so the suppression rule cannot run and the cached path emits findings the cold
path suppresses. Measured on a 146-file test repository: 626 L1 groups exist,
512 survive suppression, so the split schema would invent **114 phantom
findings** — a cache that changes the answer, which is worse than no cache.

So one physical fragment is one row, carrying both hashes:

```sql
CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT NOT NULL);
CREATE TABLE file (path TEXT PRIMARY KEY, content_hash TEXT NOT NULL);
CREATE TABLE fragment_occurrence (
  path TEXT NOT NULL,
  seq INTEGER NOT NULL,        -- emission ordinal; preserves walk order
...
The key is `(path, seq)` and not `(path, start, end)`, because two AST nodes
can share a byte range exactly: with ASI, `const x = f(a, b)` produces a
VariableStatement and a VariableDeclarationList with identical bounds, and both
are real fragments. On the same test repository 434 of 6,740 fragments (6.4%)
collide that way, and keyed on the range each collision drops a fragment on the
warm path only.

`meta` also holds a `schema_version`. A database written by a different version
is dropped and rebuilt rather than read.

### The test that matters

`tests/cache-pipeline.test.ts` runs the full duplication analysis three ways —
uncached, cold-with-cache, warm-from-cache — and asserts all three cluster
lists are deeply equal, order included. It fails on both mistakes above. The
report-level equivalence test does **not**: the report is truncated to the top
40 findings, so a cache that loses 6% of fragments can still render an
identical report on a small fixture. Assert on the clusters, not the Markdown.
...
- Rows for files that vanished are purged. They can never contaminate a report
  — the loop drives off the project's file list, not off the table — but left
  alone the database grows forever.
...
```bash
git add src/cache src/fingerprint/shape.ts tests/cache.test.ts tests/cache-pipeline.test.ts
git commit -m "feat: add the content-addressed cache and wire it into the pipeline"
```
...
Run it against a real multi-package TypeScript project outside this repo:
...
- [ ] `npm run build` — clean
- [ ] Report on a real multi-package project names real duplication, and the top findings are **not** import boilerplate
