...
describe("findVariants", () => {
  it("links two findings whose shapes differ by an inserted statement", () => {
    ...
    const a = input("UB-DUP-a");
    const b = input("UB-DUP-b", {
    ...
    expect(links.get("UB-DUP-a")).toEqual([
      { id: "UB-DUP-b", similarity: 0.8125, copies: 19 },
    ]);
    // Both directions, so whichever the reader reaches first points at the other.
    expect(links.get("UB-DUP-b")?.[0]?.id).toBe("UB-DUP-a");
    expect(links.get("UB-DUP-b")?.[0]?.copies).toBe(5);
  });

  it("does not link findings that merely share the language's phrasing", () => {
    const a = input("UB-DUP-a", { tokens: tokens(60) });
    const b = input("UB-DUP-b", {
  ...
  it("never links a fragment to its own ancestor, however alike they are", () => {
    ...
    const outer = input("UB-DUP-outer", {
      occurrences: [{ filePath: "same.ts", start: 0, end: 500 }],
      tokens: shared,
    });
    const inner = input("UB-DUP-inner", {
  ...
  it("links findings in the same file that do not overlap", () => {
    ...
    const first = input("UB-DUP-first", {
      occurrences: [{ filePath: "same.ts", start: 0, end: 100 }],
    });
    const second = input("UB-DUP-second", {
      occurrences: [{ filePath: "same.ts", start: 200, end: 300 }],
      tokens: tokens(60, ["x"]),
    });
    expect(findVariants([first, second]).get("UB-DUP-first")?.[0]?.id).toBe("UB-DUP-second");
  });
  ...
  it("treats a fragment too short to shingle as similar to nothing", () => {
    ...
    const a = input("UB-DUP-a", { tokens: ["x", "y"] });
    const b = input("UB-DUP-b", {
      tokens: ["x", "y"],
  ...
  it("orders variants by similarity and caps them", () => {
    ...
    const subject = input("UB-DUP-subject", { tokens: base });
    ...
    const links = findVariants([subject, ...others]).get("UB-DUP-subject")!;
    expect(links).toHaveLength(3);
    expect(links.map((v) => v.id)).toEqual(["UB-DUP-o0", "UB-DUP-o1", "UB-DUP-o2"]);
  ...
  it("is deterministic when two variants are equally similar", () => {
    ...
    const subject = input("UB-DUP-subject");
    const twins = ["UB-DUP-zzz", "UB-DUP-aaa"].map((id) =>
      input(id, { tokens: shape, occurrences: [{ filePath: `${id}.ts`, start: 0, end: 100 }] }),
    );
    const forward = findVariants([subject, ...twins]).get("UB-DUP-subject")!;
    const reverse = findVariants([subject, ...twins.reverse()]).get("UB-DUP-subject")!;
    expect(forward.map((v) => v.id)).toEqual(["UB-DUP-aaa", "UB-DUP-zzz"]);
