...
- **Walking an AST spine, follow what you are looking for, not what you are not.** `isBareCall` descends through wrappers to find the call, and it first selected "the one child that is not an identifier". A `VariableDeclaration` holds the binding name, the **type annotation**, and the initializer, so `const matter: Matter = await f(…)` had two candidate children and was rejected. That is most annotated call sites in a TypeScript codebase. Enumerating type node kinds is a list to keep in step with the language. Asking which child contains the call needs no list.
- **Correct reuse has the same AST as copy-paste, and only arithmetic tells them apart.** A finding read "43 copies · ~208 lines recoverable" over 43 route handlers. Each one opens with a single call to a shared helper. A reader who checks that finding and finds it already done discounts the other 58. The test is not a judgement: the shortest thing that can replace a call is a call. It holds only at L0, where every copy is the same text. It never holds for a fragment carrying a body, a literal, a template, or a second call. Twenty identical `useEffect(() => { … })` blocks are a hook waiting to be written, and the body is what makes them extractable. It took 2 of 165 candidates. Two things about wiring a *filter* into a ranker, both of which reached review broken. **A filter placed after a truncation costs slots.** The re-ranking pool is the top `slots × 3` candidates, and suppressing inside it silently shrinks it. On a fixture whose three highest candidates are all bare calls, one slot emitted an empty report with real duplication one place below the cut. Fill the pool to *survivors*. **And the report has to say which mechanism removed what.** The Omitted preamble said every unshown finding ranks below the shown ones. A rule-removed candidate can rank above them, and the same test proves it, so the report contradicted itself. **A count taken from the pool is not a count taken from the page.** The pool is three times the slots, so most of what a filter removes there would have lost the truncation anyway. Printing the pool count tells a reader that `--include-call-sites` will show them that many more findings, and it will not. Rank the removed candidates against the survivors and count only those that reach a printed slot.
- **"The L0 stream is the text" is only true for kinds whose text the stream keeps, and the exceptions do not announce themselves.** `isBareCall` rests on it: at L0 every copy is the same text, so there is nothing to parameterize. `TemplateHead` and `JsxText` are not literal kinds, so their content never enters the stream at all, and `render(<div>a</div>)` matches `render(<div>b</div>)` with no literal token between them. Both were found by review, one after the other. Match on the kind PREFIX rather than on a list, because the next such kind will not be on the list either.
- **A guard meant to be conservative can delete the feature it protects.** `true`, `false` and `null` carry no text, so a colon test cannot see them, and treating every one of them as configuration is the obvious conservative fix. It also un-suppressed the 43-copy finding the whole module exists for, whose last argument is `userRole: user.role ?? null`. That `null` is an operand of `??`, not a configured value. Position decides it, not presence: a keyword under a `PropertyAssignment` or a call argument is data, a keyword inside an expression is not. Re-run the motivating case after every tightening, because a rule with nothing left to fire on still passes its unit tests.
- **Some duplication is a framework's API, and the filename says so.** Storybook CSF wants one `const meta` per story file. Two such findings held production slots on a real report. A cluster is a file-role convention when every file shares a dotted role suffix. It must also carry at most one more occurrence than it has files. That second half is the one that is easy to leave out. A Drizzle `updatedAt` column sat in 39 `.schema.ts` files, one role suffix and 100% of them, but 56 times with seven in one file. That shape is per table, and a shared column helper absorbs every copy. Never for test-majority clusters. `.test.ts` passes every condition, and one `vi.mock` block per test file is scaffolding a setup file can absorb. Tests are held out of production slots by a **section**. Stacking a weight on top re-penalizes them, and reorders the test section on a rule that does not apply there. Two files at least: two copies in ONE role-suffixed file pass `files + 1` and there is no "across files" to report. And print the slack the rule actually allows. "Declared once in each of 44 files" is false for the 45th copy. A field named for something it does not hold is worse than no field, because it is believed. The role must be a **dotted** suffix. Reading a whole basename as a role makes every `types.ts` in a repository one role, and it down-weighted a real finding over seven sibling `resolver-types.ts` files. That is why `page.tsx` and `route.ts` are not matched, and why the docs must not claim they are.
- **Two findings over the same files are one job, and nothing in the report was saying so.** Ten of 59 findings described one structure. Seven sibling files repeated eight shapes between them, and two findings covered the identical seven files. The reader rebuilt the grouping by hand. `subsume` cannot see it, because it drops a fragment covered at the same location. These are different shapes side by side. Keyed on set **containment**, not similarity. File-set Jaccard over those 59 findings decayed smoothly from 1.0 with no empty band, so every threshold was arbitrary. Containment needs no number and says something actionable. Visiting the covering finding's files reaches every copy of the covered one. 23 of the 174 overlapping pairs nest. **A symmetric field cannot order a two-directional relation.** Both sides report the *contained* set's size, because both sentences need it. Sorting the cap on that number leaves the id as the only tie-break for a finding contained by several others, which drops the broadest relative. That is the one holding the most context. Order on the other finding's size and keep the shared count for the text.
