...
- **A proposed fix that changes nothing that runs is worse than no proposal.** Type-only edges were *preferred* as cuts on the reasoning that moving a types file is cheap — and produced a two-symbol cut an agent executed in ten minutes and correctly called a no-op. Likewise a cut for an SCC no file-level cycle underlies removes no cycle that exists. Both are now refused outright rather than ranked low.
